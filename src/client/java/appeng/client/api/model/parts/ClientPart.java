...
public record ClientPart(PartModel.Unbaked model, Properties properties) {
     ...
     *
     * @param requireCableConnection A solid part indicates that the rendering requires a cable connection, which will also result in
    ...
    public record Properties(boolean requireCableConnection) {
        ...
        public static final MapCodec<Properties> MAP_CODEC = RecordCodecBuilder.mapCodec(
                builder -> builder.group(Codec.BOOL.optionalFieldOf("require_cable_connection", true).forGetter(Properties::requireCableConnection))
