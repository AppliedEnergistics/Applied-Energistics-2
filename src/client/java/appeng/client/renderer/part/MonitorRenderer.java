...
public class MonitorRenderer implements PartRenderer<AbstractMonitorPart> {
    @Override
    public void renderDynamic(AbstractMonitorPart part, float partialTicks, PoseStack poseStack, MultiBufferSource buffers, int packedLight, int packedOverlay, Vec3 cameraPosition) {
        if (!part.isActive()) {
            return;
        }
        ...
        BlockEntityRenderHelper.renderItem2dWithAmount(poseStack, buffers, configuredItem, part.getAmount(), part.canCraft(),
