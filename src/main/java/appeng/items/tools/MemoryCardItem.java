...
public class MemoryCardItem extends AEBaseItem implements IMemoryCard, AEToolItem {
    ...
    public void notifyUser(Player player, MemoryCardMessages msg) {
        ...
        switch (msg) {
            case SETTINGS_CLEARED -> player.sendMessage(PlayerMessages.SettingCleared.text(), Util.NIL_UUID);
            case INVALID_MACHINE -> player.sendMessage(PlayerMessages.InvalidMachine.text(), Util.NIL_UUID);
            case SETTINGS_LOADED -> player.sendMessage(PlayerMessages.LoadedSettings.text(), Util.NIL_UUID);
            case SETTINGS_SAVED -> player.sendMessage(PlayerMessages.SavedSettings.text(), Util.NIL_UUID);
            case SETTINGS_RESET -> player.sendMessage(PlayerMessages.ResetSettings.text(), Util.NIL_UUID);
            default -> {
            }
        }
    }
