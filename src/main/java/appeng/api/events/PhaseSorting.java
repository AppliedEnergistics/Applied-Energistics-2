...
public class PhaseSorting {
    ...
    /**
     * Deterministically sort a list of phases. 1) Compute phase SCCs (i.e. cycles). 2) Sort phases by id within SCCs.
    ...
    private static <T> void forwardVisit(EventPhaseData<T> phase, EventPhaseData<T> parent,
            List<EventPhaseData<T>> toposort) {
        if (phase.visitStatus == 0) {
            // Not yet visited.
            phase.visitStatus = 1;
