...
/**
 * Interface implemented by the various AE2 tile entities that connect to the grid, to support callbacks from the
 * tile entities main grid node.
 */
public interface IGridConnectedTileEntity extends IActionHost, IOwnerAwareTile {
    ...
    /**
     * Used to break the tile when the grid detects a security violation.
     * Implemented in {@link AEBaseTileEntity#securityBreak()}
     */
    void securityBreak();

    /**
     * Used to save changes in the grid nodes contained in the tile entity to disk.
     * Implemented in {@link AEBaseTileEntity#saveChanges()}
     */
    void saveChanges();

    /**
     * Called when the tile entities main grid nodes power or channel assignment state changes.
     * Primarily used to send rendering updates to the client.
     */
