...
// Helper class for recipe viewer integration with auto-filling the config slots in parts like
// interfaces or storage buses.
public class FilterTransferHelper<T extends UpgradeableMenu<? extends IUpgradeableObject>> {
    ...
    private List<FakeSlot> getFakeSlots(T menu) {
        List<FakeSlot> slots = new ArrayList<>(
                menu.getSlots(SlotSemantics.CONFIG).stream().map(s -> (FakeSlot) s).toList());
        if (!slots.isEmpty()) {
            return slots;
        }
        // `AEBaseMenu#getSlots` does not always work correctly with menus from addons,
        // but if it fails we can still do things the ugly way.
        LOGGER.info(
                "Attempting to recipe fill into a menu ({}) that has not defined its SlotSemantics!",
