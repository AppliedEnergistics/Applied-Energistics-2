...
public class EntropyManipulatorCategory extends ViewBasedCategory<EntropyRecipe> {
    ...
    @Override
    public Class<? extends EntropyRecipe> getRecipeClass() {
        ...
        if (fluid != null) {
            ...
            if (!fluid.isSource(fluid.defaultFluidState())) {
                if (fluid instanceof FlowingFluid flowingFluid) {
                    slot.addIngredient(ForgeTypes.FLUID_STACK, new FluidStack(flowingFluid.getSource(), 1000));
                } else {
                    // Don't really know how to get the source :-(
                    slot.addIngredient(ForgeTypes.FLUID_STACK, new FluidStack(fluid, 1000));
