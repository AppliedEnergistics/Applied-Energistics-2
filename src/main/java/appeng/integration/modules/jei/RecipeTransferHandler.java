    ...
    public final IRecipeTransferError transferRecipe(T container, Object recipe, IRecipeLayout recipeLayout,
        ...
        if (doTransfer) {
            if (canSendReference) {
                ...
                // To avoid earlier problems of too large packets being sent that crashed the client,
                // as a fallback when the recipe ID could not be resolved, we'll just send the displayed
                // items.
                NonNullList<Ingredient> flatIngredients = NonNullList.withSize(9, Ingredient.EMPTY);
                ItemStack output = ItemStack.EMPTY;

                // Determine the first JEI slot that has an actual input, we'll use this to offset the
                // crafting grid target slot
                int firstInputSlot = recipeLayout.getItemStacks().getGuiIngredients()
                        .entrySet()
                        .stream()
                        .filter(e -> e.getValue().isInput())
