...
public class EntropyRecipeSerializer implements IRecipeSerializer<EntropyRecipe> {
    ...
    private static <T> T getRequiredEntry(Registry<T> registry, String id) {
        ...
        if (entry == null) {
            throw new IllegalArgumentException("Unknown id " + id + " for " + registry.getRegistryKey().getLocation());
        }
        return entry;
    }
    ...
    public EntropyRecipe read(ResourceLocation recipeId, PacketBuffer buffer) {
        ...
        if (buffer.readBoolean()) {
            Block inputBlock = readRegistryIdUnsafe(buffer, Registry.BLOCK);
        ...
        if (buffer.readBoolean()) {
            Fluid fluid = readRegistryIdUnsafe(buffer, Registry.FLUID);
            builder.setInputFluid(fluid);
            int matcherSize = buffer.readInt();
            for (int i = 0; i < matcherSize; i++) {
        ...
        if (buffer.readBoolean()) {
            Block block = readRegistryIdUnsafe(buffer, Registry.BLOCK);
            builder.setOutputBlock(block);
    ...
    public void write(PacketBuffer buffer, EntropyRecipe recipe) {
        ...
        if (recipe.getInputBlock() != null) {
            writeRegistryIdUnsafe(buffer, Registry.BLOCK, recipe.getInputBlock());
        ...
        if (recipe.getInputFluid() != null) {
            writeRegistryIdUnsafe(buffer, Registry.FLUID, recipe.getInputFluid());
        ...
        if (recipe.getOutputBlock() != null) {
            writeRegistryIdUnsafe(buffer, Registry.BLOCK, recipe.getOutputBlock());
        ...
        if (recipe.getOutputFluid() != null) {
            writeRegistryIdUnsafe(buffer, Registry.FLUID, recipe.getOutputFluid());
    ...
    private static <T> void writeRegistryIdUnsafe(PacketBuffer buffer, DefaultedRegistry<T> registry, T entry) {
        buffer.writeVarInt(registry.getId(entry));
    }

    private static <T> T readRegistryIdUnsafe(PacketBuffer buffer, DefaultedRegistry<T> registry) {
        return registry.getByValue(buffer.readVarInt());
    }
