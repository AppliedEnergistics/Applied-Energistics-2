...
public class PartPlacement {
    ...
    public static ActionResultType place(final ItemStack held, final BlockPos pos, Direction side,
        ...
        if (!held.isEmpty() && Platform.isWrench(player, held, pos) && Platform.isEntityHoldingShift(player)) {
        ...
        if (held.isEmpty()) {
            if (host != null && Platform.isEntityHoldingShift(player) && world.isAirBlock(pos)) {
                if (mop.getType() == RayTraceResult.Type.BLOCK) {
        ...
        if (!world.isRemote) {
            if (mop.getType() != RayTraceResult.Type.MISS) {
                ...
                if (sp.part != null) {
                    if (!Platform.isEntityHoldingShift(player) && sp.part.onActivate(player, hand, mop.getHitVec())) {
                        return ActionResultType.FAIL;
                    }
                }
            }
    ...
    public void playerInteract(final PlayerInteractEvent event) {
        ...
        if (event instanceof PlayerInteractEvent.RightClickEmpty && event.getPlayer().world.isRemote) {
            ...
            if (mop instanceof BlockRayTraceResult && mop.getHitVec().distanceTo(vec3) < d0) {
                ...
                if (Platform.isEntityHoldingShift(event.getPlayer()) && !held.isEmpty() && supportedItem) {
