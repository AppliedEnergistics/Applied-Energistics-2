...
public final class DispenserBehaviorTinyTNT extends BehaviorDefaultDispenseItem
{
	...
	{
		final Direction Direction = dispenser.getBlockState().getValue( BlockDispenser.FACING );
		final World world = dispenser.getWorld();
		final int i = dispenser.getBlockPos().getX() + Direction.getFrontOffsetX();
		final int j = dispenser.getBlockPos().getY() + Direction.getFrontOffsetY();
		final int k = dispenser.getBlockPos().getZ() + Direction.getFrontOffsetZ();
