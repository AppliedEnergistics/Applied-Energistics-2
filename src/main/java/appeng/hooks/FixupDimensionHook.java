...
/**
 * This fixes an issue with DFU that essentially trips over the spatial storage dimension if it happens
 * to be the first dimension it sees in the iteration order of the dimension compound tag, and a version update
 * (i.e. 1.18 -> 1.18.1) happens to be applied.
 * <p/>
 * It does this by removing the AE2 dimension from the level.dat NBT before it is processed by DFU, and
 * re-adding it before the actual level data is deserialized. This will also fix cases where the data is
 * missing from level.dat entirely.
 */
