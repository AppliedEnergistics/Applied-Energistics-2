...
public class ContainerPatternTerm extends ContainerMEMonitorable implements IAEAppEngInventory, IOptionalSlotHost, IContainerCraftingPacket
{
	...
	public void encode()
	{
		...
		else if ( output == null )
		{
			...
			output = AEApi.instance().definitions().items().encodedPattern().stack( 1 );
	...
	private boolean isPattern(ItemStack output)
	{
		...
		boolean isPattern = definitions.items().encodedPattern().sameAsStack( output );
		isPattern |= definitions.materials().blankPattern().sameAsStack( output );
