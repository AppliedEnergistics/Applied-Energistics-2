...
public class PatternAccessTermScreen<C extends PatternAccessTermMenu> extends AEBaseScreen<C> {
    ...
    private final HashMap<Long, PatternProviderRecord> byId = new HashMap<>();
    // Used to show multiple pattern providers with the same name under a single header
    private final HashMultimap<String, PatternProviderRecord> byName = HashMultimap.create();
    ...
    public void drawFG(PoseStack poseStack, int offsetX, int offsetY, int mouseX,
        ...
        for (; i < this.numLines; ++i) {
            if (scrollLevel + i < this.lines.size()) {
                ...
                if (lineObj instanceof PatternProviderRecord inv) {
                    ...
                    for (int z = 0; z < inv.getInventory().size(); z++) {
                        this.menu.slots
                                .add(new PatternSlot(inv, z, z * SLOT_SIZE + GUI_PADDING_X, (i + 1) * SLOT_SIZE));
                    }
    ...
    public void drawBG(PoseStack poseStack, int offsetX, int offsetY, int mouseX,
        ...
        for (int i = 0; i < this.numLines; ++i) {
            ...
            if (scrollLevel + i < this.lines.size()) {
                final Object lineObj = this.lines.get(scrollLevel + i);
                isInvLine = lineObj instanceof PatternProviderRecord;
            }
    ...
    private void refreshList() {
        ...
        for (PatternProviderRecord entry : this.byId.values()) {
        ...
        for (String n : this.names) {
            ...
            List<PatternProviderRecord> clientInventories = new ArrayList<>(this.byName.get(n));
    ...
    private PatternProviderRecord getById(long id, long sortBy, Component name) {
        PatternProviderRecord o = this.byId.get(id);

        if (o == null) {
            this.byId.put(id,
                    o = new PatternProviderRecord(id, PatternProviderLogic.NUMBER_OF_PATTERN_SLOTS, sortBy, name));
            this.refreshList = true;
        }
