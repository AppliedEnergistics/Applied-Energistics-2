...
public abstract class AEBaseGui extends GuiContainer
{
	...
	// drag y
	final Set<Slot> dragClicks = new HashSet<Slot>();
	...
	public void setScrollBar( GuiScrollbar newScrollBar )
	{
	...
	public void drawScreen( int mouseX, int mouseY, float btn )
	{
		...
		{
			if( button instanceof ITooltip )
			{
				ITooltip tooltip = (ITooltip) button;
				int x = tooltip.xPos(); // ((GuiImgButton) button).xPosition;
				int y = tooltip.yPos(); // ((GuiImgButton) button).yPosition;
	...
	protected void mouseClickMove( int x, int y, int c, long d )
	{
		...
		{
			this.dragClicks.add( slot );
			if( this.dragClicks.size() > 1 )
			{
				for( Slot dr : this.dragClicks )
				{
	...
	protected void handleMouseClick( Slot slot, int slotIdx, int ctrlDown, int key )
	{
		...
		if( !this.disableShiftClick && isShiftKeyDown() )
		{
			...
			if( this.doubleClickStack == null || this.doubleClickSlot != slot || this.doubleClickTimer.elapsed( TimeUnit.MILLISECONDS ) > 150 )
			{
				// some simple double click logic.
				this.doubleClickSlot = slot;
				this.doubleClickTimer = Stopwatch.createStarted();
				if( slot != null )
					this.doubleClickStack = slot.getHasStack() ? slot.getStack().copy() : null;
				else
					this.doubleClickStack = null;
			}
			else if( this.doubleClickStack != null )
			{
				...
				{
					if( inventorySlot != null && inventorySlot.canTakeStack( this.mc.thePlayer ) && inventorySlot.getHasStack() && inventorySlot.inventory == slot.inventory && Container.func_94527_a( inventorySlot, this.doubleClickStack, true ) )
					{
