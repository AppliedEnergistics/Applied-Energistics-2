...
This demonstration setup is cut short so it isn't too wide, it is likely most optimal to use 4 <ItemLink id="interface" />s and 4 <ItemLink id="storage_bus" />es,
...
<BoxAnnotation color="#dddddd" min="0 1 0" max="2 1.3 1">
        (2) Storage Buses: "Input/Output Mode" set to "Extract Only".
  </BoxAnnotation>
...
* The <ItemLink id="storage_bus" />es (2) are set such that "Input/Output Mode" is set to "Extract Only".
...
2. The <ItemLink id="storage_bus" />es allow the network to access the contents of the interfaces.